...
단어 풀은 9개 언어를 모두 지원하고, 그 위에 있는 `randWord`와 29개 테마 함수와 `randNickname`도 마찬가지입니다. 언어마다 다른 것은 수식어가 어디에 놓이는지, 그리고 명사 옆에서 모양이 바뀌는지입니다. 둘 다 각 언어가 자기 프레임과 일치 규칙에 적어 둡니다.
