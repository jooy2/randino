...
단어 풀은 — 그리고 그 위에 있는 `randWord`, 25개 테마 함수, `randNickname`은 — 9개가 아니라 7개 언어만 지원하며, 이유는 작업량이 아니라 문법입니다. 수식어는 사전에 실린 그대로 명사 옆에 놓여야 하는데, 이 방식은 둘 사이에 문법적 일치를 요구하지 않는 언어에서만 자연스럽게 읽힙니다.
...
아직 빠진 두 언어는 같은 이유가 한 걸음 더 간 경우입니다. **러시아어와 독일어**도 굴절하는데, 이들의 형태는 수식어를 명사 앞에 둡니다(`blauer Wal`). 일치가 아직 닿지 못하는 유일한 경우입니다. 맞춰야 할 명사보다 수식어를 먼저 뽑기 때문입니다.
