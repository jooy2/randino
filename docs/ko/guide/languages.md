...
단어 풀은 — 그리고 그 위에 있는 `randWord`, 25개 테마 함수, `randNickname`은 — 9개가 아니라 8개 언어만 지원하며, 이유는 작업량이 아니라 문법입니다. 수식어는 사전에 실린 그대로 명사 옆에 놓여야 하는데, 이 방식은 둘 사이에 문법적 일치를 요구하지 않는 언어에서만 자연스럽게 읽힙니다.
...
**러시아어** 하나만 남았고, 이유는 풀을 아직 쓰지 않았다는 것뿐입니다. 어미 규칙은 독일어와 같은 방식으로 들어갑니다.
