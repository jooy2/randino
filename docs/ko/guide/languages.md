...
어순도 문법 일치도 더 이상 언어를 막는 이유가 아닙니다. 형태가 언어에 붙어 있으므로 베트남어는 수식어를 명사 뒤에, 소유의 대상을 주인 앞에 둡니다. 굴절하는 언어는 명사마다 성을 태그로 달고 수식어가 바뀌는 어미를 목록으로 적어 두므로, 스페인어는 `gato dorado` 옆에 `luna dorada`를 어느 쪽도 두 번 저장하지 않고 씁니다. 단수형이 아예 없는 명사(`ножницы`, `gafas`, `Jeans`)는 성 대신 복수를 답니다. 단수 수식어가 그 옆에 설 수 없기 때문입니다.
