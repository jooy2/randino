...
모든 옵션에 기본값이 있어서 인자 없이 호출해도 동작하며, [지원하는 9개 언어](./languages) 중 하나로 된 이름 하나가 돌아옵니다. 기본값은 여러 언어를 섞는 것입니다. 샘플 데이터에는 알맞지만 한 언어로 통일해야 하는 화면에는 맞지 않으니, 그럴 때는 언어를 지정하면 됩니다.
