...
문자열 하나, 또는 배열 안의 모든 문자열 앞에 무작위 토큰을 붙입니다. [`randSuffix`](./rand-suffix)를 앞쪽으로 뒤집은 것으로, 구분자가 앞에 와야 하는 값에 씁니다. 샤드나 테넌트, 정렬 기준이 따로 없는 키 같은 것입니다. 토큰은 `randSuffix`와 같은 것이고, [무엇에 쓰면 안 되는지](./rand-suffix#not-for-security)도 같습니다.
...
| 옵션 | 타입 | 기본값 | 설명 |
| --- | --- | --- | --- |
| `value` | `string \| string[]` | — | 앞에 붙일 대상. 옵션이 아니라 첫 번째 인자. 생략하면 토큰만 |
...
| 인자 | 타입 | 기본값 | 설명 |
| --- | --- | --- | --- |
| `value` | `str \| list[str] \| None` | `None` | 앞에 붙일 대상. 위치 인자이며 나머지는 키워드 전용. 생략하면 토큰만 |
| `length` | `int` | `5` | 토큰의 글자 수. `1` … `32`로 제한 |
