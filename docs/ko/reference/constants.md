...
생성기가 받아들이는 목록과, 모든 숫자 옵션이 제한되는 절대 범위입니다. 범위를 벗어난 값에 예외를 던지지는 않습니다. `count`가 `-10`이면 아무것도 반환하지 않고, 100만이면 1만 개를 반환합니다.
...
<Lang js="charset" dart="charset" py="charset" code />으로 좁히거나 넓힐 수 있습니다. 알파벳 전체가 아니라 기본값에서 출발해 줄이면 이 성질이 유지됩니다.
...
`…Option`으로 끝나는 타입은 언어나 테마에 `'all'`을 더한 유니온입니다. `NameLanguageOption`은 `NameLanguage | 'all'`입니다. `'all'`이 유효한 답이 아닌 자리에서는 더 좁은 쪽을 쓰면 되고, 헬퍼가 그렇게 하고 있습니다.
