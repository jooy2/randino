...
일상 단어를 만들어 `count`개만큼 문자열로 돌려줍니다. 동물, 사물, 자연, 개념 등 25개의 [테마](./themes)를 8개 언어로 제공하며, **사람 이름은 절대 쓰지 않습니다.** [`output: 'detail'`](#the-detail-output)을 주면 각 단어의 언어와 테마까지 알려 줍니다.
