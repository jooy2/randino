...
`type`은 문장이 무엇을 하는지 정합니다. `'statement'`, `'question'`, `'exclamation'`, 그리고 끝나는 대신 멈추는 `'trailing'`, 여기에 아래의 인용 두 가지입니다. **지정하지 않으면 문장마다 여섯 중 하나를 뽑습니다.** 하나를 지정하거나, 여러 개를 묶거나, `'all'`을 줄 수 있습니다.

이 뽑기는 **고르지 않고 한쪽으로 기울어 있습니다.** 글이 그렇기 때문입니다. 평서문이 압도적으로 많고, 누군가 하는 말이 그다음이며, 의문과 감탄이 여섯 중 가장 드뭅니다. 앞뒤가 의문문이 아닐 때라야 의문문이 읽을 만해집니다. 종류를 직접 지정하면 그 종류만 나옵니다.
...
`'dialogue'`와 `'thought'`는 형태가 아예 아닌 두 타입입니다. 둘 다 문장을 그 언어의 따옴표로 감싸고, 감싸는 대상은 줄마다 새로 뽑힙니다. 말하는 사람은 글보다 자주 묻지만, 그래도 묻기보다 말하는 쪽이 많습니다.
...
**사람은 부류가 아닙니다.** 민준에 관한 문단은 민준에 관한 것입니다. 이름을 다시 부르거나 그 자리에 대명사를 세울 뿐, 같은 부류의 다른 사람을 뽑지 않습니다. 그랬다면 문단이 슬그머니 다른 사람 이야기가 됩니다.

**장면도 유지합니다.** 첫 문장이 부른 장소가 나머지 문장이 벌어지는 곳이고, 다루던 사물이 계속 다루는 사물입니다. 뒤 문장은 자기 관사와 수식어를 새로 쓰므로 `안개 나루터에서`가 `나루터에서`가 되기는 해도 `금고에서`가 되지는 않습니다.

**어투도 처음 것을 지킵니다.** 말을 서술하는 글은 말이 되지 않으므로, 서술로 연 문단에는 인용이 섞이지 않습니다. 인용으로 연 문단은 대사 사이에 서술이 들어갑니다. 두 사람이 말할 때는 번갈아 하고, 한 사람이 내리 네 마디를 하는 문단은 그동안 벌어진 일을 통째로 빼먹은 문단이기 때문입니다. 사이에 들어가는 서술은 제3의 목소리가 되어서는 안 되므로 묻지 않고 말합니다.

**접속사는 두 문장이 감당할 수 있는 것만 주장합니다.** 접속사마다 앞 문장에 대해 무엇을 말하는지 표시해 두었고, 넷 중 셋은 어떤 문장 뒤에도 올 수 있습니다. 시간이 흘렀다, 하나 더 있다, 그것과 반대다. 넷째는 다릅니다. `그러므로`, `그래서`, `so`, `だから`는 이 문장이 앞 문장에서 **비롯된다**고 말하는데, 그건 둘이 같은 것을 다루고 이 문장이 묻는 대신 말할 때만 참입니다. 프레첼 이야기 뒤에 오는 `그러므로 금빛 하이볼이 식죠?`는 아무것도 뒤따르지 않는 결과입니다.
...
언어는 자기가 쓸 수 있는 종류만 선언합니다. 독일어에는 시간을 나타내는 것이 없습니다. `dann`과 `danach`는 부사이고, 부사가 첫 자리에 서면 정동사가 움직이기 때문에, 절을 열 수 있는 것은 등위접속사 다섯 개뿐입니다.

**한 문단에서 같은 것을 두 번 하지 않습니다.** 바로 앞 문장과 같은 종류는 되풀이될수록 확률이 낮아지므로 의문문이 줄줄이 이어지다 저절로 끊깁니다. 동사 무리는 한 바퀴 다 쓰고 나서야 다시 쓰므로 네 문장 중 셋이 같은 서술어로 끝나지 않습니다. 사람 이름은 한 번 부르고 나면 한동안 부르지 않습니다. 새로 뽑는 주어는 부류 전체가 아니라 화제와 같은 테마에서 주로 나옵니다. 그리고 한 결과 안에서 같은 접속사나 감탄사로 두 문장을 열지 않습니다.

대명사를 어떻게 쓸지는 언어마다 다릅니다. 영어는 `it`을 쓰고, 한국어·일본어·중국어·스페인어·이탈리아어는 주어를 아예 생략합니다. 두 번째 문장에서 실제로 그렇게 쓰기 때문입니다. 독일어와 러시아어는 명사의 성에 따라 고릅니다. 사람을 가리킬 수 없는 대명사만 가진 언어 — 영어의 `he`/`she`는 단어 풀에 없는 성이 필요하고 `그것`, `それ`, `它`, `nó`는 무생물입니다 — 는 대신 화제를 다시 부릅니다.

**이름은 예외입니다.** 성을 가진 주어는 이름뿐이기 때문입니다. `Philip counts the prime. He forgets it in the supernova.`처럼, `the locksmith`라면 다시 부를 수밖에 없던 자리에 영어가 `he`를 세웁니다. 성별 대명사가 없는 언어는 그대로입니다. 한국어는 사람을 두고 `그것`이라 하는 대신 주어를 생략합니다.
