...
화계는 어미만이 아니라 서법까지 바꿉니다. 해체와 해요체는 의문형과 감탄형이 따로 없습니다. `달려`는 묻는 말도 되고 하는 말도 되며 뒤에 붙는 문장부호만 다르므로, 이 둘은 모든 서법에 형태 하나를 씁니다. 해라체와 합쇼체는 형태가 서로 다르게 바뀝니다. 해라체는 `-니`, `-나`, `-(으)ㄴ가`로 묻고 `-구나`, `-네`, `-군`으로 감탄하며, 합쇼체는 `달립니다`를 `달립니까`로 바꿉니다.
