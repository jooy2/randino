...
## 범위가 넓은 이유 {#why-the-range-is-so-wide}

아래쪽 끝은 가장 짧은 형태에 가장 짧은 단어를 넣은 것이고, 위쪽 끝은 가장 긴 형태의 모든 구에 수식어를 붙인 것입니다. 그 사이 어디에 떨어지든 그 언어가 쓸 수 있는 문장이며, 어느 것이 나올지는 범위의 양 끝이 아니라 형태의 가중치가 정합니다.
