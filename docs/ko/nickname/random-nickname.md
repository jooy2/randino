...
| 옵션 | 타입 | 기본값 | 설명 |
| --- | --- | --- | --- |
| `language` | <Lang js="NicknameLanguageOption" dart="NicknameLanguage?" code /> | <Lang js="'all'" dart="null" code /> | 생성할 닉네임의 언어. <Lang js="'all'" dart="null" code />이면 지원하는 모든 언어를 섞어서 닉네임마다 하나씩 고릅니다. |
| `theme` | <Lang js="NicknameThemeOption" dart="NicknameTheme?" code /> | <Lang js="'all'" dart="null" code /> | 닉네임의 주제. [테마](./themes)를 참고하세요. |
| `count` | <Lang js="number" dart="int" code /> | `1` | 반환할 닉네임의 개수. `0` … `10000` 범위로 제한됩니다. |
| `style` | <Lang js="number" dart="int" code /> | `0` | `0`은 실제 단어를, `100`은 그 언어처럼 읽히기만 하는 단어를 만들어 씁니다. 중간값은 둘을 섞습니다. |
| `minLength` | <Lang js="number" dart="int?" code /> | _언어별_ | 최소 글자 수. 고유 접미사는 **세지 않습니다**. |
| `maxLength` | <Lang js="number" dart="int?" code /> | _언어별_ | 최대 글자 수. 고유 접미사는 **세지 않습니다**. |
| `includeModifier` | <Lang js="boolean" dart="bool" code /> | `true` | 단어를 꾸밉니다. `사자`가 아니라 `멋진사자`. |
| `wordSeparator` | <Lang js="string" dart="String?" code /> | _언어별_ | 단어 사이에 넣습니다. 길이 범위에 포함됩니다. 기본값은 붙여 쓰는 것입니다. |
| `baseWord` | <Lang js="string" dart="String?" code /> | — | 모든 닉네임을 이 단어를 중심으로 만들고 장식만 바꿉니다. |
| `uniqueSuffix` | <Lang js="boolean" dart="bool" code /> | `false` | 무작위 토큰을 붙여서 두 사람이 같은 닉네임을 갖지 않게 합니다. |
| `uniqueSuffixLength` | <Lang js="number" dart="int" code /> | `5` | 토큰의 글자 수. `1` … `32` 범위로 제한됩니다. |
| `uniqueSuffixSeparator` | <Lang js="string" dart="String" code /> | `'_'` | 닉네임과 토큰 사이에 넣습니다. 빈 문자열이면 바로 이어 붙입니다. |
| `uniqueSuffixCharset` | <Lang js="string" dart="String?" code /> | _내장값_ | 토큰을 뽑을 문자 집합. 기본값은 헷갈리는 쌍(`0O`, `1lI`)을 뺀 영숫자입니다. |
| `startsWith` | <Lang js="string" dart="String?" code /> | — | 첫 글자가 이 글자인 닉네임만 반환합니다. |
| `unique` | <Lang js="boolean" dart="bool" code /> | `false` | 같은 닉네임을 두 번 반환하지 않습니다. 조합이 바닥나면 `count`보다 적게 반환할 수 있습니다. |
...
뒤따르는 단어는 여전히 허용됩니다. `includeModifier`는 수식어가 들어가는 형태만 제외할 뿐, 하나만 남기는 옵션이 아닙니다.
...
구분자의 길이도 `minLength` / `maxLength`에 포함됩니다. [`nicknameLengthRange`](./nickname-length-range)에 넘겨 보면 남은 범위를 확인할 수 있습니다.
...
접미사는 `minLength` / `maxLength`를 만족한 **뒤에** 덧붙습니다. 그래서 이 옵션들은 읽는 부분만을 가리키고, 접미사가 그 범위를 잠식하지 않습니다.
