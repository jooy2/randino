...
`realism`은 <Lang js="theme: 'all'" dart="theme을 비운 경우" py="theme=&quot;all&quot;" code />가 어떤 테마를 도는지도 정합니다. 25개 가운데 `color`, `finance`, `tech` 셋은 닉네임으로 어색합니다. 색이나 대출 앞에 수식어가 붙으면 핸들이 아니라 농담으로 읽히기 때문입니다(`멋진대출`, `BraveInvoice`, `奔跑的服务器`). `real`에서는 이 셋이 빠지고 `mixed`와 `invented`에서는 들어옵니다. 테마를 직접 지정하면 `realism`과 무관하게 언제나 그 테마가 나옵니다.
