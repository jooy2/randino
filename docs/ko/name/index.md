...
공백으로 구분되는 언어에서는 풀에서 다시 뽑아 범위를 맞추고, 그래도 들어오지 않으면 범위에 닿을 수 있는 길이에서만 각 요소를 뽑습니다. 그래서 풀이 담을 수 있는 최댓값은 지켜지고, 아예 닿을 수 없는 범위는 최댓값을 넘기는 대신 최솟값에 못 미치는 이름으로 답합니다. 한국어, 일본어, 중국어는 이름을 음절 단위로 조합하기 때문에 범위를 정확히 맞춥니다.
