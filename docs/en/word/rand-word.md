...
Generates everyday words and returns `count` of them as strings. Animals, things, nature, ideas — twenty-five [themes](./themes), in six languages, and **never a person name**. With [`output: 'detail'`](#the-detail-output) it reports the language and theme behind each word.
