...
| `vocabulary` | `RandVocabulary` | <Lang js="`'full'`" dart="`RandVocabulary.full`" py="`\"full\"`" /> | How common the nouns have to be — the subject, the object, the place, the thing a story is about: `basic`, `common` or `full`. See [Common words](../word/rand-word#vocabulary). |
