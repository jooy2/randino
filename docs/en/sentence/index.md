...
## The words of one sentence belong together

A sentence built by picking words at random is a pile of words. What keeps this one together is the verb: every verb states which kinds of noun can do it, and — when it is transitive — which kinds it can be done to. The nouns are then drawn from those alone.
...
## The shapes belong to the language
...
`shape` is the same idea in coarser units — how many phrases rather than how many characters — and is usually what a caller means by a short or a long sentence.

## What `realism` changes, and what it does not
...
`수줍은 노오가 안개 파저멜을 챙긴다.` is not Korean vocabulary, and it is Korean grammar.
