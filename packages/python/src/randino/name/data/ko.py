...
# Conventional romanization of the surnames below. Anything missing (including
# a syllable passed through `starts_with`) falls back to Revised Romanization.
KO_SURNAME_ROMAN = roman_map("""
    ...
    소:So 석:Seok 선:Sun 마:Ma 표:Pyo 태:Tae 명:Myung 기:Ki 반:Ban 왕:Wang 금:Geum
    옥:Ok 육:Yook 인:In 맹:Maeng 제:Je 모:Mo 탁:Tak 국:Kook
""")
...
KO = NameLanguageData(
    ...
    given_male=words("""
        ...
        훈 빈 찬 진 결 온 솔 강 산 담 별 건 태준 도준 민우 재우 지운 성훈 시훈 은호
        서호 정후 도경 시환 재환 태영 상훈 병철 영철 성수 종수 재석 병수 동석 영진
        상호 진성 남준 형우 경훈 동훈 민규 승규 명수 광민 대호 기훈 우석 재호 준석
        현석 영민 로운 리안 유건 은결 진영 성용 종원 정욱 동건 근우 두호 호진 경수
        상수 민태 태주 태하 민석 이재 재성 재인 강훈 인규 남규 정남 순재 영우 우영
        민건 인권 대겸 수호 준오 강인 기찬 찬우 철민 철수 철규 광진 성광
    """),
    given_female=words("""
        ...
        다빈 유은 하진 소희 예빈 태연 나래 미주 솔 별 봄 온 결 린 슬 담 진 윤 이서
        서은 지윤 예윤 하율 시율 다율 채윤 아라 시하 은유 서우 라온 아율 지효 세희
        유선 소현 미현 은혜 영숙 미숙 미란 혜경 혜영 혜선 수경 은미 정미 나영 다영
        소정 예림 유림 아영 소혜 가연 도희 서정 유미 진아 선희 경아 은아 지연 소진
        예슬 하빈 서빈 채연 소민 주연 연아 민아 민주 연지 소유 수희 희수 희연 연희
        승아 나라 가현 수지 나리 수미 정아 채민 라희 종서 현아 현서 연주 동은 은숙
        명숙 명서 명선 성연 이현 미연 미희
    """),
    first_male=words("""
    ...
    """),
    rest_female=words("""
        연 윤 우 은 서 유 아 원 율 린 빈 진 하 영 지 희 나 슬 미 경 정 담 별 혜 효 림 라 현
    """),
)
