import type { NameDetail, RandomNameOptions } from '../_types/global.js';
import { generateNameDetails } from './nameGenerator.js';

/**
 * Generate person names with both scripts and the choices behind each one.
 *
 * Takes the same options as `randomName`, except `script` — every name is
 ...
 */
export function randomNameDetails(options: RandomNameOptions = {}): NameDetail[] {
