...
export const IT: SentenceLanguageData = {
	...
	verbs: [
		...
		// What somebody shows, split by what it shows — see the Korean data.
		{
			...
			condition: 'content',
			...tensed(
				`ride sorride canticchia ridacchia fischietta esulta annuisce applaude fa_l'occhiolino scoppia_a_ridere sorride_a_trentadue_denti`,
				`rise sorrise canticchiò ridacchiò fischiettò esultò annuì applaudì fece_l'occhiolino scoppiò_a_ridere sorrise_a_trentadue_denti`
			)
		},
		{
			field: 'express',
			...
			condition: 'restless',
			...tensed(
				`piange sospira borbotta singhiozza geme brontola piagnucola sbuffa balbetta fa_una_smorfia scuote_la_testa aggrotta_la_fronte mugugna si_morde_il_labbro si_gratta_la_testa cammina_avanti_e_indietro`,
				`pianse sospirò borbottò singhiozzò gemette brontolò piagnucolò sbuffò balbettò fece_una_smorfia scosse_la_testa aggrottò_la_fronte mugugnò si_morse_il_labbro si_grattò_la_testa camminò_avanti_e_indietro`
			)
		},
		{
			field: 'express',
		...
		},
		{
			...
			subject: ['creature', 'person'],
			...tensed(
				`grida esclama ansima si_stringe_nelle_spalle starnutisce arrossisce strilla saluta_con_la_mano inclina_la_testa sgrana_gli_occhi sussulta`,
				`gridò esclamò ansimò si_strinse_nelle_spalle starnutì arrossì strillò salutò_con_la_mano inclinò_la_testa sgranò_gli_occhi sussultò`
			)
		},
		{
			field: 'talk',
	...
	traits: {
		...
		gabbiano coccinella cherubino viverna gargolla
		`),
		swimmer: words(`
		...
		lifeless: words(`
			...
			paradiso inferno purgatorio olimpo aldilà limbo pozione elisir malocchio maleficio scongiuro magia
			stregoneria alchimia calderone mandragola
		`),
		...
		placeless: words(`
			...
			fango polvere pietra ghiaia argilla vortice lava magma fuoco fiamma scintilla riflesso raggio bagliore
			bolla goccia ghiaccio valanga frana eruzione maremoto alluvione incendio ragnatela zodiaco radiazione
			traiettoria allunaggio decollo missione alieno apogeo congiunzione propulsore etere astronomia astrologia
			nova magnetar brillamento occultazione radiotelescopio cosmologia astrofisica
		`)
	},
	interjections: words(`
	...
	replies: {
		casual: {
			agree: words(`
				già certo è_vero anch'io proprio_così senza_dubbio infatti esatto eccome lo_credo_bene
			`),
			cheer: words(`
				che_bello! ben_fatto! stupendo! che_fortuna fantastico! complimenti che_gioia! magnifico! finalmente! mi_fa_piacere
			`),
			care: words(`
				stai_bene? riposati_un_po' non_strafare mangiamo_qualcosa che_peccato fai_con_calma non_preoccuparti stai_attento coraggio siediti_un_attimo bevi_un_po'_d'acqua ti_aiuto_io
			`),
			wonder: words(`
				davvero? sul_serio? dove? quando? e_poi? non_ci_credo! come? perché? ah_sì? e_dopo? cos'è_successo? cosa?
			`),
			answer: words(`
				sì,_un_po' no,_sto_bene sì,_parecchio così_così no,_non_ancora sì,_molto un_pochino non_tanto sì,_moltissimo no,_per_niente insomma sì,_in_effetti
			`)
		}
	},
