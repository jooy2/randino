...
lib/
  ...
  word/
    ...
    randAnimal.ts …         # public: one per theme, twenty-nine of them, one doc page each
...
- **A name narrows the top of the budget and never its floor.** `roomFor` puts `nameSpan` in the subject's slot, which is what stops a shape being chosen for a range only a noun phrase could have reached — a name is one word and no article. It also makes the shortest shape shorter, and `boundsFor` measures its low end against `slotBounds` for that reason: `sentenceLengthRange` is a promise about the language, and `彤找。` at three characters is Chinese saying it writes four. Every noun slot has its own entry in `slotBounds` so that narrowing one narrows one; reading a shared `noun` for all of them is how the narrowing went quietly dead in the JavaScript package while both ports had it.
...
- **What never changes is worked out once.** Five answers are held rather than recomputed: what a shape can be (`frameRange`, keyed on the shape and the bounds it was measured against), how long the language's articles are, which traits a noun carries, which theme holds a word, and whether a story is tellable about a hero — `itemThemesFor` asks the last of those once per theme, on every result. They are all functions of the datasets, which are module constants, so the caches key on identity and never grow. **A change that makes one of them depend on the draw has to take its cache with it**, and `roomFor` is the shape of that mistake avoided: it hands back one object per language rather than a fresh one per sentence, because `frameRange` remembers against the object it was handed.
- **A verb pool is written under three constraints the suites only partly see.** German moves the whole verb token to the front of a question, so a separable prefix is wrong there (`Bricht auf der Fuchs?`): a German verb is one word or a reflexive (`erhebt sich`), never `bricht auf` or `wird still`. Russian agrees its past with the subject's gender by rewriting `л` / `лся` at the end of the token, so every Russian past is an imperfective form ending on them — `уходил`, never `ушёл`, `рос` or `затих`, and never a token closing on a noun. And the Japanese suite tells a plain sentence from a polite one by its ending, so no Japanese predicate may spell one at a plain level, in either tense: not `目を覚ます`, and not `こだまする`, whose plain past `こだました` is the polite past of a verb nobody has. `no plain Japanese predicate closes the way a polite one does` reads the pools for it. English and Vietnamese carry their destination head in the frame (`to`, `đến`), so a verb that carries its own (`makes for`) doubles it.
...
- **Countries are every country, in every word language, and divisions are not.** `randCountry` draws from `countries.ts`: the 249 codes of ISO 3166-1 as the tz database lists them (public domain), named by Wikidata's labels (CC0), so its `language` is a `WordLanguageOption` and it answers in all nine. Any list of countries takes a position on a few places, and ISO's is the one every country picker already uses; a list of the world's *divisions* takes a position on far more (Crimea, Kashmir, a province of Taiwan), which is why `randRegion` and below stay with the countries whose own lists are free of that. `randLocation({ level: 'country' })` is still the language's own country, and the name it writes is read out of the same table (`KR`, `US`), so the two cannot spell it differently.
...
5. Aim for 100+ nouns per theme, and more where the vocabulary is there. Every language holds around 3,500 to 3,800 nouns, and the thinnest theme in any language sits in the sixties. The pools are what make the output varied, and the combination count is the sum over the language's frames of the product of their slots — 44M to 166M for the five with a `parts` pool, 99M to 106M for `es` and `it`, and 1.1M for `de` and `ru`, whose three frames are the shortest of any language. **Padding a theme with near-synonyms reads worse than a shorter pool**, and inventing a compound to fill it is how `棒麺麭` and `대로변` got in; both were replaced. See the compound rule below — a pool grows by finding words the theme does not have yet, never by qualifying one it already holds.
...
5. Add the `rand<Theme>` function beside the other twenty-nine, export it from `lib/word/index.ts`, and add it to the `THEMED` table in `test/word.test.ts` — that table is asserted to have exactly one entry per theme, so a missing function fails the suite. Update the theme list in `README.md` and the doc comment on `WordTheme`.
