...
- **The default range is wide on purpose** (`nicknameLengthRange('ko')` is `[1, 13]`): it spans every shape, and the pattern weights — not the range — decide what output usually looks like.
...
2. Add `lib/word/data/<code>.ts` with a `WordLanguageData` object: `joiner`, `capitalize`, `adjectives` and `actions` in attributive form, `nouns` for every theme in `WORD_THEMES`, an optional `parts` pool, and a `syn` template (`kind: 'syllable'` for alphabetic scripts, `kind: 'pool'` where one character is one syllable).
