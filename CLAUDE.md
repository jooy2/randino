...
- **A quantity is sometimes the subject.** `사과 12개가 익는다` has no separate subject, so `subjectSlotOf` and `takesObject` read the shape rather than looking for a `subject` part. Without that, the counted subject is drawn from the verb's *object* classes and Korean produces `카푸치노 4개가 먹는다`. `numeral.amounts` is a pool rather than a range, because a range hands back `73,412 dollars`; `numeral.group` is per language, because Vietnamese, Spanish and Italian group on a full stop.
