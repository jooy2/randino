...
- **API**, which nests three groups by what a function *does with a string*: **Generators** make one out of nothing (`randName`, `randNickname`, `randWord`), **Decorators** attach something to one you already have (`randSuffix`, `randPrefix`, `randModifier`), and **Utilities** answer a question about a language (`nameLengthRange`, `wordLengthRange`, the two `nameSupports…`). Generators is kept short on purpose, so that a new kind of random text is visible when it arrives — which is why the fourteen themed word functions sit in **Words** underneath `randWord` rather than beside it.
...
`data/sidebar.ts` nests as deep as it is written: a `SidebarGroup`'s `items` are pages, or more groups, and `sidebarFor` recurses. **Two levels is the working limit** — API > Generators > Words is the deepest entry there is, and it earns the third level by being a function's own arguments rather than a category: `randAnimal` is `randWord`, so it belongs under it. Anything that is merely *related* to a page goes beside it, not under it, or the menu stops being a menu.
...
Those fourteen are the **Words** group nested inside Generators, under the `randWord` they all are — beside it they would bury the three generators in a list of seventeen. They are also the one group the navbar's API dropdown leaves out, and it falls out rather than being configured: `navGroupsFor` reads a group's pages and not its subgroups. The Markdown that is left on each page is what actually differs: what the theme is, and three code samples of it.
